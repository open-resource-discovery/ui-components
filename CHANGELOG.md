...
- Icon buttons: extracted a lean `IconButton` primitive (two swappable icons + `onClick`, no state) that `CopyButton` now composes, and added `LinkButton` (chain icon — navigates via `LinkRender`/`href`) and `ExternalLinkButton` (square-with-arrow icon — opens in a new tab). All share the `buttonVariants` surface.
